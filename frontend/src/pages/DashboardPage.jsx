...
import { Grid, Package, Boxes, Loader2, BarChart3, Clock, TrendingUp, AlertTriangle, PlusCircle, Pencil, ScanLine, X } from 'lucide-react';
...
const StatCard = ({ title, value, IconComponent, color, navigateTo }) => {
    ...
    return (
        <div onClick={() => navigate(navigateTo)} className={`p-5 md:p-6 bg-white rounded-2xl shadow-md flex items-center justify-between transition duration-300 hover:cursor-pointer hover:shadow-xl border-l-4 ${classes.border}`}>
            <div>
                <p className="text-xs md:text-sm font-semibold text-gray-500 uppercase tracking-wider">{title}</p>
                <h2 className={`text-2xl md:text-4xl font-extrabold text-gray-900 mt-1 ${classes.text}`}>{formattedValue}</h2>
            </div>
    ...
    return (
        <li className="flex items-start space-x-3 p-3 hover:bg-gray-50 rounded-xl transition duration-150 border-b border-gray-50 last:border-0">
            ...
            <div className="grow min-w-0">
                ...
                <p className="text-[10px] uppercase font-bold text-gray-400 mt-1 tracking-tight">{time}</p>
            </div>
        </li>
    );
};
...
const DashboardPage = () => {
    ...
    const activityFeedData = [
        { icon: PlusCircle, color: { text: "text-green-600", bg: "bg-green-100" }, description: "<b>+50 items</b> received into <span class='text-indigo-700 font-medium'>Warehouse A</span>", time: "1 hour ago" },
        { icon: AlertTriangle, color: { text: "text-red-600", bg: "bg-red-100" }, description: "Product <b>'XYZ-123'</b> is out of stock", time: "3 hours ago" },
        { icon: Pencil, color: { text: "text-blue-600", bg: "bg-blue-100" }, description: "Price updated for <b>'Premium Faucet'</b>", time: "Yesterday" },
    ];
    ...
    if (loading && !stats) {
        return (
            <div className="flex flex-col items-center justify-center p-6 min-h-[60vh]">
                ...
                <span className="mt-4 text-lg font-medium text-gray-600">Syncing Inventory...</span>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="m-4 p-6 text-center bg-red-50 rounded-2xl border border-red-200">
                ...
                <p className="text-sm text-red-600 mt-1">{error.message || "Please check your internet."}</p>
            </div>
        );
    }
    ...
    return (
        ...
        <div className="bg-gray-50 p-4 md:p-8 min-h-screen">
            ...
            <header className="mb-6 md:mb-10">
                ...
                <p className="text-gray-500 text-sm mt-1 ml-1">Real-time stock overview</p>
                    ...
                    <ul className="space-y-1">
                        {activityFeedData.map((activity, index) => (
                            <ActivityItem 
                                key={index} 
                                {...activity}
                                description={<span dangerouslySetInnerHTML={{ __html: activity.description }} />}
                            />
                        ))}
                    </ul>
